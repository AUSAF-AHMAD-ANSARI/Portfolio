  ...
  return (
    <nav className="w-full z-99999 transition-all duration-300">
      <div className="flex justify-between items-center px-[8%] lg:*:px-[16%] py-6">
        <Link
          href="/"
          className="text-2xl font-bold font-unbounded text-white logo "
        >
          Portfolio
          <span className="text-(--primary-color) font-unbounded">Hub.</span>
        </Link>
        {/*Desktop Nav*/}

        <div className="flex items-center gap-3">
          <div className="hidden lg:flex nav-menu items-center space-x-3">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
          ...
          <Link
            href="/Contract"
            className="bg-(--primary-color) px-5 py-2 text-xs text-white whitespace-nowrap font-semibold cursor-pointer rounded-full transition-all duration-300 hover:bg-transparent shadow-md hover:shadow-[0px_2px_5px_var(--primary-color)] "
          >
            Hire Me
          </Link>
        </div>
